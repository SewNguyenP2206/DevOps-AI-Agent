...
func HanldeGitPullRequest(input string, memory *[]string) {
	reader := bufio.NewReader(os.Stdin)

	prompt := fmt.Sprintf(`
You are an AI assistant. The user wants to pull the latest changes from a Git repository.
Request: "%s"
Here is the current memory:
%s
Your task:
1. Identify the Git repository URL from the memory.
2. Identify the directory where the user wants to pull the repo.
Respond ONLY with JSON in the format:
{
  "url": "https://...",
  "directory": "/path/to/pull"
}
No explanation, no markdown.
`, input, strings.Join(*memory, "\n"))
	resp, err := llm_tool.AskLLM(prompt)
	...
	if _, err := os.Stat(result.Directory); os.IsNotExist(err) {
		fmt.Println("❗ Directory does not exist. Cannot pull changes.")
		return
	}
	// Run git pull
	fmt.Println("🔄 Pulling latest changes...")
	cmd := exec.Command("git", "-C", result.Directory, "pull")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
	...
	fmt.Println("✅ Successfully pulled latest changes from:", result.Directory)
	// Update memory
	newFact := fmt.Sprintf("Pulled latest changes from %s in %s", result.URL, result.Directory)
	*memory = append(*memory, newFact)
	if err := memory_func.SaveMemory("devOpsmemory.txt", *memory); err != nil {
		fmt.Println("⚠️ Failed to save memory:", err)
	} else {
